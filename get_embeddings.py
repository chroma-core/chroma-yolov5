# This script shows how to use Chroma with YOLOv5, a popular object detection model.
# Each input image may have zero or more detections, and each detection has an embedding.
# The embeddings are logged to Chroma, along with the input image URI, the inference class name, and the label class name (if available).
...
# Associate detections to labels. This function is used in training mode.
def associate_preds_labels(preds, targets, im, shapes, iou_thres=0.5):
    ...
    for i, pred in enumerate(preds):
        ...
        all_associations.append(
            [
                labels[best_preds.index(i), 0] if i in best_preds_set else None
...
# Process a batch of images, and log to Chroma
def process_batch(
    im,
    targets,
    shapes,
    ...
    # NMS
    with dt[2]:
        preds, embeddings = non_max_suppression(
            preds,
            conf_thres,
            iou_thres,
            classes,
    ...
    with dt[3]:
        ...
        if targets is not None:
            ...
            label_class_names = [
                names[int(a)] if a is not None else None
                for a in chain.from_iterable(associations)
            ]
        ...
        # Log to Chroma
        chroma_client.add(
            embedding=torch.cat(embeddings, 0).tolist(),
            input_uri=paths,
            inference_class=inference_class_names,
            label_class=label_class_names if targets is not None else None,
            dataset=dataset_name,
            model_space=chroma_client.get_model_space(),
        )

    # Print profiling times
    LOGGER.info(
        f"Image Transform time: {dt[0].dt:.3f}s, Inference time: {dt[1].dt:.3f}s, NMS time: {dt[2].dt:.3f}s, Chroma time: {dt[3].dt:.3f}s"
    )
...
def run(
    ...
    dataloader = (
        create_dataloader(data[dataset], imgsz, batch_size, stride, workers=workers)[0]
        if with_labels
        else create_imageloader(
            data[dataset], imgsz, batch_size, stride, workers=workers
        )[0]
    )
...
def parse_opt():
    ...
    parser.add_argument(
        "--dataset-name", type=str, default="", help="dataset name for chroma storage"
    )
    ...
    parser.add_argument(
        "--with-labels", action="store_true", help="Associate labels to get label class"
    )
